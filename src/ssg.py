...
class SystemStateGraph:
    ...
    def create_window_graph(self, syscalls: List[str], arguments: List[List[str]]) -> nx.DiGraph:
        """Create a graph for a single time window."""
        window_graph = nx.DiGraph()
        
        # Process each syscall and its arguments
        for syscall, args in zip(syscalls, arguments):
            # Add syscall node
            syscall_node = f"syscall_{syscall}"
            window_graph.add_node(syscall_node, type='syscall')
    ...
    def update_graph(self, df_window: pd.DataFrame):
        ...
        for _, row in df_window.iterrows():
            ...
            window_graph = self.create_window_graph(syscalls, arguments)
    ...
    def get_graph_stats(self) -> Dict:
        ...
        return {
            ...
            'syscall_nodes': len([n for n, attr in self.graph.nodes(data=True) 
                                if attr.get('type') == 'syscall']),
            'argument_nodes': len([n for n, attr in self.graph.nodes(data=True) 
                                 if attr.get('type') == 'argument'])
        }

    def visualize(self, output_file: str = 'system_state_graph.png'):
        ...
        syscall_nodes = [n for n, attr in self.graph.nodes(data=True) 
                        if attr.get('type') == 'syscall']
        nx.draw_networkx_nodes(self.graph, pos, nodelist=syscall_nodes, 
                             node_color='lightblue', node_size=500)
        
        # Draw argument nodes
        arg_nodes = [n for n, attr in self.graph.nodes(data=True) 
                    if attr.get('type') == 'argument']
        nx.draw_networkx_nodes(self.graph, pos, nodelist=arg_nodes, 
        ...
        # Add labels
        labels = {node: node.split('_', 1)[1] for node in self.graph.nodes()}
        nx.draw_networkx_labels(self.graph, pos, labels, font_size=8)
        
        plt.title('System State Graph')
        plt.axis('off')
    ...
    def extract_features(self) -> Dict[str, Any]:
        ...
        Uses KNOWN_SYSCALLS and KNOWN_ARGUMENTS from preprocessing as baseline.
        """
        features = {}
        ...
        # Define baseline thresholds (you might want to adjust these values)
        BASELINE_THRESHOLD = 5  # Consider frequency abnormal if it's 5x the average
        
        if syscall_counts:
            # Calculate average frequency across all syscalls
            avg_frequency = sum(syscall_counts.values()) / len(syscall_counts)
            
            # Check if any syscall frequency is abnormally high compared to average
            max_increase = max(
