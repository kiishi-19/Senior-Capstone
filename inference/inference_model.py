...
def process_file(file_path, models, thresholds, batch_size, device):
    try:
        # Load dataset
        ...
        # Save anomaly scores and classifications using pickle
        output_scores = os.path.splitext(file_path)[0] + "_anomaly_scores.pkl"
...
def process_files_in_parallel(file_paths, models, thresholds, batch_size, device):
    with Pool() as pool:
        results = pool.starmap(
            process_file,
            [(file, models, thresholds, batch_size, device) for file in file_paths]
        )
    return results
...
if __name__ == "__main__":
    ...
    parser.add_argument("--thresholds", nargs="+", type=float, default=[0.14860, 1.1148, 1.4864], help="Anomaly thresholds")
    args = parser.parse_args()

    activation_fn = nn.Sigmoid if args.activation == "sigmoid" else nn.ReLU

    # Load a sample file to determine the feature dimension
    sample_file = glob(os.path.join(args.inference_data_dir, '*.pkl'))[0]
    feature_dim = pd.read_pickle(sample_file).shape[1]
    ...
    process_files_in_parallel(file_paths, models, args.thresholds, args.batch_size, args.device)
