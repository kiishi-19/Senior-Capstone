# multiprocessing__process_scap_files.py
...
def process_single_scap_file(scap_file):
    """
    Process a single .scap file and extract features.
    """
...
def process_scap_directory(scap_dir, output_file=None):
    """
    Process all .scap files in the given directory and extract features using multiprocessing.
    ...
    print(f"Processing {len(scap_files)} .scap files using {cpu_count()} cores...")

    # Process files in parallel
    with Pool(cpu_count()) as pool:
        results = list(tqdm(pool.imap(process_single_scap_file, scap_files), total=len(scap_files), desc="Processing .scap files"))

    # Flatten the list of lists into a single list
    feature_list = [item for sublist in results if sublist is not None for item in sublist]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process .scap files and extract features using multiprocessing.")
